...
def collect_news(log_func=print) -> list:
    ...
    for url in RSS_FEEDS:
        try:
            feed = feedparser.parse(url, request_headers=REQUEST_HEADERS, timeout=10)
...
class NewsApp(App):
    def __init__(self, **kwargs):
        ...
        self.font_name = None  # will be set if font file exists

    def build(self):
        Window.clearcolor = (0.95, 0.97, 0.99, 1)

        # Try to register Persian-capable font if present
    ...
    def try_register_font(self):
        # Put Vazirmatn-Regular.ttf next to main.py
    ...
    def on_start(self):
        try:
            ...
            self.safe_status(f"Startup error: {e}")
    ...
    def handle_back_button(self):
        ...
        if now - self.last_back_press_time < 1.5:
            ...
            self.safe_status("Press back again to exit.")
        return True
    ...
    def on_reset_mails_pressed(self, instance):
        try:
            ...
            self.save_stats()
            self.update_total_label()
            self.safe_status("Mail history reset. Total emails sent: 0.")
        except Exception as e:
            self.safe_status(f"Error resetting mails: {e}")
        finally:
    ...
    def on_sender_delete(self, index):
        ...
        self.safe_status("Sender deleted.")
        self.save_config()
    ...
    def save_config(self):
        ...
        if not cfg["senders"]:
            self.safe_status("At least one sender is required.")
            return
        if not cfg["recipients"]:
            self.safe_status("At least one recipient is required.")
            return
        ...
        try:
            ...
            self.safe_status(f"Error saving settings: {e}")
    ...
    def load_config(self):
        ...
        try:
            ...
            self.safe_status(f"Error loading settings: {e}")
            self.on_add_recipient()
    ...
    def on_send_pressed(self, instance):
        ...
        self.safe_status("Starting to send...")
    ...
    def send_news_flow(self):
        try:
            ...
            all_items = collect_news(log_func=self._log_thread_safe)
