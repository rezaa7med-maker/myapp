...
def send_emails_safe(sender_email, app_password, to_emails, news_items, progress_cb=None):
    ...
    for verified in (True, False):
        try:
            ...
            return True, (
                "Sent with verified SSL" if verified else "Sent without SSL verification"
            )
...
class NewsApp(App):
    ...
    def _background_app(self):
        # به جای بستن کامل، اپ را به پس‌زمینه بفرست
        if platform == "android":
            try:
                from jnius import autoclass
                PythonActivity = autoclass("org.kivy.android.PythonActivity")
    ...
    def build(self):
        ...
        # ارتفاع لیبل با متن تنظیم شود تا اسکرول درست کار کند
        self.status_label.bind(texture_size=lambda inst, *_: setattr(inst, "height", inst.texture_size[1]))
        ...
        content.add_widget(status_scroll)
    ...
    def on_keyboard(self, window, key, scancode, codepoint, modifier):
        if key == 27:
            ...
            if now - self._last_back_time < 1.2:
                ...
                self.set_status("برای خروج، دوباره دکمه برگشت را بزنید.")
            return True
        return False
    ...
    def show_menu_popup(self):
        ...
        exit_btn.bind(on_release=lambda *_: (popup.dismiss(), self._background_app()))
        popup.open()
    ...
    def set_status(self, text):
        ...
        # اطمینان از بروزرسانی ارتفاع برای اسکرول
        Clock.schedule_once(lambda *_: self.status_label.texture_update(), 0)
