...
class NewsApp(App):
    ...
    # ---- new helper for background-exit (requirements 3 & 4) ----
    ...
    def build(self):
        ...
        self.exit_toast.bind(
            size=lambda inst, *_: setattr(inst, "text_size", (inst.width, inst.height))
        )
        root.add_widget(self.exit_toast)
    ...
    def show_exit_toast(self):
        ...
        def hide_toast(dt):
            self.exit_toast.opacity = 0

        self._toast_ev = Clock.schedule_once(hide_toast, 3)
    ...
    def show_menu_popup(self):
        ...
        reset_btn = Button(
            ...
            height=BTN_HEIGHT,
            background_normal="",
            background_color=(1.0, 0.8, 0.3, 1.0),
            color=(0, 0, 0, 1),
        )
        exit_btn = Button(
            text="Exit",
    ...
    def show_sender_form_popup(self, edit_index=None):
        ...
        # ✅ فقط همینجا اصلاح شد: padding بالا بیشتر شد
        wrapper = BoxLayout(
            orientation="vertical",
            spacing=dp(10),
            padding=[dp(12), dp(22), dp(12), dp(12)],  # left, top, right, bottom
        )
