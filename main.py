...
class NewsApp(App):  
    ...
    def build(self):  
        ...
        self.status_label.bind(texture_size=lambda inst, val: setattr(inst, "height", val[1]))  
  
        self.status_scroll = ScrollView(  
            size_hint=(1, None),  
            height=dp(220),  
            do_scroll_x=False,  
        )  
        self.status_scroll.add_widget(self.status_label)  
        content.add_widget(self.status_scroll)  
    ...
    def on_keyboard(self, window, key, scancode, codepoint, modifier):  
        if key == 27:  
            now = time.time()  
            if now - self._last_back_time <= 1.5:  
    ...
    def show_menu_popup(self):  
        ...
        exit_btn.bind(on_release=lambda *_: (popup.dismiss(), self._move_to_background()))  
        popup.open()  
            ...
            try:  
                ...
                for t, s in items:  
                    if t not in seen:  
                        ...
                        seen.add(t)  
                available_feeds = len(unique_titles)
                total_sent = len(self.sent_titles)
                remaining_new = len([t for t, _ in unique_titles if t not in self.sent_titles])  
                Clock.schedule_once(  
                    lambda dt: (  
                        self.set_status(  
                            f"RSS OK\n"  
                            f"Availabe feeds: {available_feeds}\n"  
    ...
    def do_send(self):  
        ...
        def task():  
            try:  
                ...
                success_count = 0  
                sent_per_sender = {}  
                total_overall = len(self.senders) * len(batch_items)
                for idx, s in enumerate(self.senders):  
                    ...
                    if not sender_email or not app_pass:  
                        ...
                        continue  
                    def progress_cb(cur, tot, se=sender_email, si=idx):  
                        overall_now = si * tot + cur
                        Clock.schedule_once(  
                            lambda dt, _se=se, _c=cur, _t=tot, _o=overall_now, _to=total_overall: self.set_status(  
                                f"Sending...\n{_se}\nEmail {_c}/{_t}\nOverall {_o}/{_to}"  
                            ),  
                            0,  
                        )  
                ...
                total_emails_sent_all = sum(sent_per_sender.values())
                out = (  
                    "Finished.\n"  
                    f"Total emails sent (all senders): {total_emails_sent_all}\n"
